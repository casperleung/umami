...
export async function createAuthToken(userId: string, role?: string, password?: string) {
  // Bind token to password hash so a password change invalidates old tokens.
  const pwd = password ? hash(password) : undefined;

  if (redis.enabled) {
    return saveAuth({ userId, role, pwd });
  }

  return createSecureToken({ userId, role, pwd }, secret());
}
