...
test.describe('User API tests', () => {
  ...
  test('returns all users when admin access is used', async ({ request }) => {
    ...
    expect(body.data[0]).not.toHaveProperty('twoFactorSecret');
    expect(body.data[0]).not.toHaveProperty('twoFactorRecoveryCodes');
    expect(body.data[0]).not.toHaveProperty('twoFactorChallenge');
